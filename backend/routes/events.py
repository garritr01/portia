...
def listEvents(uID):
	...
	logger.info(f"GET events in range {_isoToDt(start)} - {_isoToDt(end)}")
	...
	if end:
		events = [e for e in events if e.get("startStamp") and e["startStamp"] < _isoToDt(end)]

	return jsonify(_objsToIso(events)), 200
