...
export const DropSelect = ({ options = [], value, setter, allowType = false }) => {
...
export const InfDropSelect = ({ min = -9999, max = 9999, buffer = 10, value, setter, allowType = false }) => {
	const { smallScreen = false } = useScreen() || {};
	const { currentNav } = useKeyNav() || {};
...
const DropView = ({ isOpen, lastOpen, options, value, setter, rVal, setRVal, allowType, headRef, chevRef, listRef, lensRef, scrollHandler, width }) => {
	const { smallScreen = false } = useScreen() || {};
		...
		<div className="drop">
			...
			<div className="dropHeader">
				...
				{allowType ?
					<input
						value={rVal.display}
						onChange={(e) => setRVal({ display: e.target.value, value: e.target.value })}
